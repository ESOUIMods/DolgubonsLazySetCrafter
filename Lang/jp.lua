DolgubonSetCrafter = DolgubonSetCrafter or {}

DolgubonSetCrafter.lang = "jp"

DolgubonSetCrafter.localizedStrings = {}

DolgubonSetCrafter.localizedStrings.UIStrings = DolgubonSetCrafter.localizedStrings.UIStrings or  {}
DolgubonSetCrafter.localizedStrings.UIStrings.patternHeader			= "装備の部位を選択してください"
DolgubonSetCrafter.localizedStrings.UIStrings.pattern				= "装備の部位"
DolgubonSetCrafter.localizedStrings.UIStrings.comboboxHeader		= "属性"
DolgubonSetCrafter.localizedStrings.UIStrings.comboboxDefault		= "未選択"
DolgubonSetCrafter.localizedStrings.UIStrings.selectPrompt			= "<<1>>を選択してください"
DolgubonSetCrafter.localizedStrings.UIStrings.style					= "スタイル"
DolgubonSetCrafter.localizedStrings.UIStrings.level					= "レベル"
DolgubonSetCrafter.localizedStrings.UIStrings.CP					= "CP"
DolgubonSetCrafter.localizedStrings.UIStrings.genericTrait			= "特性"
DolgubonSetCrafter.localizedStrings.UIStrings.armourTrait			= "防具の特性"
DolgubonSetCrafter.localizedStrings.UIStrings.weaponTrait			= "武器の特性"
DolgubonSetCrafter.localizedStrings.UIStrings.jewelryTrait			= "宝飾品の特性"
DolgubonSetCrafter.localizedStrings.UIStrings.quality				= "品質"
DolgubonSetCrafter.localizedStrings.UIStrings.gearSet				= "セット"
DolgubonSetCrafter.localizedStrings.UIStrings.addToQueue			= "キューに追加"
DolgubonSetCrafter.localizedStrings.UIStrings.queueHeader			= "クラフト キュー"
DolgubonSetCrafter.localizedStrings.UIStrings.clearQueue			= "キューをクリア"
DolgubonSetCrafter.localizedStrings.UIStrings.resetToDefault		= "選択をクリア"
DolgubonSetCrafter.localizedStrings.UIStrings.notEnoughKnowledge	= "この属性でクラフトするのに十分な知識がありません"
DolgubonSetCrafter.localizedStrings.UIStrings.notEnoughMats			= "この属性でクラフトするのに十分な材料がありません"
DolgubonSetCrafter.localizedStrings.UIStrings.notEnoughSpecificMat	= "すべてのアイテムを作成するのに十分な材料がありません"
DolgubonSetCrafter.localizedStrings.UIStrings.invalidLevel			= "無効なレベル"
DolgubonSetCrafter.localizedStrings.UIStrings.multiplier			= "数量"
DolgubonSetCrafter.localizedStrings.UIStrings.autoCraft				= "クラフトの自動化"
DolgubonSetCrafter.localizedStrings.UIStrings.mimicStones			= "ミミックストーンを使用"
DolgubonSetCrafter.localizedStrings.UIStrings.craftStart			= "クラフト開始"
DolgubonSetCrafter.localizedStrings.UIStrings.materialScrollTitle	= "材料の要件"
DolgubonSetCrafter.localizedStrings.UIStrings.mailRequirements      = "必要な材料をメールする"
DolgubonSetCrafter.localizedStrings.UIStrings.chatRequirements      = "必要な材料をチャットする"
DolgubonSetCrafter.localizedStrings.UIStrings.chatRequest 			= "Request to Chat"
DolgubonSetCrafter.localizedStrings.UIStrings.mailRequest 			= "Mail Request"
DolgubonSetCrafter.localizedStrings.UIStrings.defaultUserId         = "@UserId を入力"
DolgubonSetCrafter.localizedStrings.UIStrings.noSet                 = "セット効果なし"
DolgubonSetCrafter.localizedStrings.UIStrings.usesMimicStone        = "このアイテムはミミックストーンを使用して作成されます"
DolgubonSetCrafter.localizedStrings.UIStrings.qualityString         = "このアイテムは <<1>> 品質で作成されます"
DolgubonSetCrafter.localizedStrings.UIStrings.armourEnchant         = "防具の付呪"
DolgubonSetCrafter.localizedStrings.UIStrings.jewelryEnchant        = "宝飾品の付呪"
DolgubonSetCrafter.localizedStrings.UIStrings.weaponEnchant         = "武器の付呪"
DolgubonSetCrafter.localizedStrings.UIStrings.enchantQuality        = "付呪の品質"
DolgubonSetCrafter.localizedStrings.UIStrings.inProgressCrafting    = "このアイテムは処理中です。キャンセルできますが、一部のアイテムはすでに作成されています"
DolgubonSetCrafter.localizedStrings.UIStrings.addSelectionFavourite = "セレクションを保存"
DolgubonSetCrafter.localizedStrings.UIStrings.addQueueFavourite		= "キューを保存"
DolgubonSetCrafter.localizedStrings.UIStrings.FavouritesTitle 		= "Favourites"
DolgubonSetCrafter.localizedStrings.UIStrings.showFavourites 		= "お気に入りの表示"
DolgubonSetCrafter.localizedStrings.UIStrings.loadQueueAsIs 		= "保存したレベルでロードする"
DolgubonSetCrafter.localizedStrings.UIStrings.loadQueueCurrentLevel	= "選択中のレベルでロードする"
DolgubonSetCrafter.localizedStrings.UIStrings.resetPatterns			= "Reset Patterns"
DolgubonSetCrafter.localizedStrings.UIStrings.totalCostTitle		= "Total Cost"
DolgubonSetCrafter.localizedStrings.UIStrings.onlyKnownRecipes		= "Only Known Recipes"

DolgubonSetCrafter.localizedStrings.SettingsStrings = {}


DolgubonSetCrafter.localizedStrings.SettingsStrings.nowEditing                   = "%sの設定を変更しています"
DolgubonSetCrafter.localizedStrings.SettingsStrings.accountWide                  = "アカウント共通"
DolgubonSetCrafter.localizedStrings.SettingsStrings.characterSpecific            = "キャラクター固有"

DolgubonSetCrafter.localizedStrings.SettingsStrings.showAtStation 				= "クラフト台で表示"
DolgubonSetCrafter.localizedStrings.SettingsStrings.showAtStationTooltip			= "クラフト台を開いたときにSet Crafterのウィンドウを表示します"
DolgubonSetCrafter.localizedStrings.SettingsStrings.saveLastChoice				= "選択を保存する"
DolgubonSetCrafter.localizedStrings.SettingsStrings.saveLastChoiceTooltip		= "ドロップダウンメニューの選択の状態を保存します"
DolgubonSetCrafter.localizedStrings.SettingsStrings.closeOnExit                  = "ウィンドウを自動的に閉じる"
DolgubonSetCrafter.localizedStrings.SettingsStrings.closeOnExitTooltip           = "クラフト台を閉じたときにSet Crafterのウィンドウも非表示にします"
DolgubonSetCrafter.localizedStrings.SettingsStrings.useCharacterSettings         = "キャラクター固有の設定を使用する"
DolgubonSetCrafter.localizedStrings.SettingsStrings.useCharacterSettingsTooltip  = "このキャラクターのみの固有の設定を使用します"
DolgubonSetCrafter.localizedStrings.SettingsStrings.showToggleButton              = "トグルボタンの常時表示"
DolgubonSetCrafter.localizedStrings.SettingsStrings.showToggleButtonTooltip       = "Set Crafterウィンドウの表示／非表示トグルボタンを常に表示します"
DolgubonSetCrafter.localizedMatScrollWidth 											= 250

DolgubonSetCrafter.localizedStrings.weaponNames =
{
--	"Axe", "Mace", "Sword", "Battle Axe", "Maul", "Greatsword", "Dagger", "Bow", "Fire Staff", "Ice Staff", "Lightning Staff", "Restoration Staff", "Shield"
	"斧", "戦棍", "剣", "両手斧", "大槌", "大剣", "短剣", "弓", "業火の杖", "氷の杖", "稲妻の杖", "回復の杖", "盾"
}
DolgubonSetCrafter.localizedStrings.pieceNames =
{
--	"Chest","Feet","Hands","Head","Legs","Shoulders","Belt","Jerkin",
	"胴体","足","手","頭","脚","肩","腰","ジャーキン",
}
DolgubonSetCrafter.localizedStrings.jewelryNames =
{
--	"Ring" , "Ring", "Necklace",
	"指輪" , "指輪", "ネックレス",
}
DolgubonSetCrafter.localizedStrings.armourTypes =
{
--	"Heavy", "Medium", "Light"
	"重装鎧", "中装鎧", "軽装鎧"
}
ZO_CreateStringId("SI_BINDING_NAME_SET_CRAFTER_OPEN", "Set Crafterウィンドウの表示／非表示")
ZO_CreateStringId("SI_BINDING_NAME_SET_CRAFTER_ADD_MAIL_ITEM", "メール内のアイテムリンクをキューに追加")
